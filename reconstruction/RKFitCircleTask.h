...
#include "Track.h"
#include "TrackLoop.h"
#include "X17Utilities.h" // IWYU pragma: keep
...
class RKFitCircleTask : public X17::RecoTask
{
  ...
  TH2F *m_h_energy, *m_h_energy2;
  TH2F *m_h_diff_energy, *m_h_diff_energy2;
  TH2F *m_h_diff_theta, *m_h_diff_theta2;
  TH2F *m_h_diff_phi, *m_h_diff_phi2;

  std::vector<double> m_v_diff_mid, m_v_diff_avg;

  std::vector<TH1F*> m_h_energy_diff;
  int m_bins;
  X17::CircleFit3D* m_cfit = nullptr;

  std::vector<TGraph2D*> m_tracks, m_fits;
  double m_ediff_max = 2e+6;

public:
  void PreTrackLoop() override
  {
    m_v_diff_mid.reserve(100000);
    m_v_diff_avg.reserve(100000);

    const char* h_energy_title = ";RK energy [MeV];Circle fit energy [MeV]";
    m_h_energy                 = new TH2F("h_energy", h_energy_title, m_bins, 3, 13, m_bins, 0, 15);
    m_h_energy2                = new TH2F("h_energy2", h_energy_title, m_bins, 3, 13, m_bins, 0, 15);

    const char* h_energy_title2 = ";RK energy [MeV];#frac{E_{rec}-E}{E} [\%]";
    m_h_diff_energy             = new TH2F("h_diff_energy", h_energy_title2, m_bins, 3, 13, m_bins, -25, 25);
    m_h_diff_energy2            = new TH2F("h_diff_energy2", h_energy_title2, m_bins, 3, 13, m_bins, -30, 10);

    const char* h_energy_title3 = ";Theta [deg];#frac{E_{rec}-E}{E} [\%]";
    m_h_diff_theta              = new TH2F("h_diff_theta", h_energy_title3, m_bins, -17.1, 17.1, m_bins, -25, 25);
    m_h_diff_theta2             = new TH2F("h_diff_theta2", h_energy_title3, m_bins, -17.1, 17.1, m_bins, -40, 10);

    const char* h_energy_title4 = ";Phi [deg];#frac{E_{rec}-E}{E} [\%]";
    m_h_diff_phi                = new TH2F("h_diff_phi", h_energy_title4, m_bins, -16.3, 16.3, m_bins, -25, 25);
    m_h_diff_phi2               = new TH2F("h_diff_phi2", h_energy_title4, m_bins, -16.3, 16.3, m_bins, -30, 10);
    ...
    m_cfit = new X17::CircleFit3D();
    m_cfit->SetFitter(4, false);
  }

  void PreElectronLoop() override
  {
    const X17::TrackRK* track = GetLoop_()->curr_rk;

    m_cfit->SetOrigOrient(track->origin, track->orientation);
    m_tracks.push_back(new TGraph2D());
    m_cfit->SetFitter(4, false);
    m_cfit->SetParameters(0, 20, 0.5, track->electron);
  }
  ...
  void ElectronLoop() override
  {
    ...
    const X17::TrackRK* track = loop->curr_rk;
    X17::RKPoint p            = loop->curr_rkpoint;

    m_cfit->AddPoint(p);
    m_tracks.back()->AddPoint(p.x(), p.y(), p.z()); // g_2d->AddPoint(p.x,p.y,p.z);
  }
  ...
  void PostElectronLoop() override
  {
    ...
    const X17::TrackRK* track = loop->curr_rk;
    std::cout << "Prefit sumsq: " << m_cfit->GetSumSq() << '\n';
    m_cfit->FitCircle3D();
    // cfit->PrintFitParams();

    // g_fit = cfit->GetGraph();

    double fit_energy_mid = m_cfit->GetEnergy(*loop->magfield, true);
    double fit_energy_avg = m_cfit->GetEnergy(*loop->magfield, false);
    ...
    double phi   = 180 / M_PI * std::acos(track->orientation.x / std::cos(std::asin(track->orientation.z)))
                   * sign(track->orientation.y);
    if (track->electron)
    {
      m_h_energy->Fill(track->kin_energy / 1e+6, fit_energy_mid / 1e+6);
      m_h_energy2->Fill(track->kin_energy / 1e+6, fit_energy_avg / 1e+6);
      m_v_diff_mid.push_back(rel_diff_mid);
      m_v_diff_avg.push_back(rel_diff_avg);

      m_h_diff_energy->Fill(track->kin_energy / 1e+6, rel_diff_mid);
      m_h_diff_energy2->Fill(track->kin_energy / 1e+6, rel_diff_avg);

      m_h_diff_theta->Fill(theta, rel_diff_mid);
      m_h_diff_theta2->Fill(theta, rel_diff_avg);
      m_h_diff_phi->Fill(phi, rel_diff_mid);
      m_h_diff_phi2->Fill(phi, rel_diff_avg);
    }

    // int index = floor(track->kin_energy / 1e+6) - 3; // Zeroth bin is 3+ MeV
    // h_energy_diff[index]->Fill((fit_energy_mid-track->kin_energy) / 1e+6);

    if (std::abs(fit_energy_mid - track->kin_energy) > m_ediff_max)
    {
      ...
      std::cout << "   orientation: (" << track->orientation.x << "," << track->orientation.y << ","
                << track->orientation.z << ")\n";
      std::cout << "   theta: " << theta << " phi: " << phi << "\n";
      std::cout << "   fit energy: " << fit_energy_mid << "\n";

      m_cfit->PrintFitParams();
      m_fits.push_back(m_cfit->GetGraph());
    }
    else
      m_tracks.pop_back();
    m_cfit->ResetData();
  ...
  void PostTrackLoop() override
  {
    ...
    double d_width       = 700;
    double d_height      = 500;
    float extra_margin_w = 0.03;
    float extra_margin_h = 0.075;
    float margin_w       = 0.1F + extra_margin_w;
    float margin_h       = 0.1F + extra_margin_h;
    float margin_top     = 0.05;

    float extra_margin_rl = 0.035;

    int width  = iround(d_width / ((0.9 - extra_margin_w - extra_margin_rl) / 0.9));
    int height = iround(d_height / ((0.9 - extra_margin_h) / 0.9));
         ...
         })
    {
      ...
      h->GetYaxis()->SetTitleOffset(x_offset - 0.05F);
      h->SetContour(256);
    }
    ...
    c->SetRightMargin(0.1F + extra_margin_rl);

    m_h_energy->Draw("colz");
    ...
    int bins_mid = GetBinsFreedmanDiaconis(m_v_diff_mid, -25, 25);
    int bins_avg = GetBinsFreedmanDiaconis(m_v_diff_avg, -20, 0);
    ...
    h_diff_mid->SetStats(false);
    h_diff_avg->SetStats(false);
    ApplyThesisStyle(h_diff_mid);
    ApplyThesisStyle(h_diff_avg);
    h_diff_mid->GetXaxis()->SetTitleOffset(x_offset);
    h_diff_mid->GetYaxis()->SetTitleOffset(y_offset + 0.2F);
    h_diff_avg->GetXaxis()->SetTitleOffset(x_offset);
    h_diff_avg->GetYaxis()->SetTitleOffset(y_offset + 0.2F);

    int width2  = iround(d_width / ((0.9 - extra_margin_w - extra_margin_rl) / 0.9));
    int height2 = iround(d_height / ((0.9 - extra_margin_h) / 0.9));

    for (double diff : m_v_diff_mid)
      h_diff_mid->Fill(diff);
    for (double diff : m_v_diff_avg)
      h_diff_avg->Fill(diff);
    ...
    // TH3F* scale = new TH3F("scale","Tracks with failed circle fit;x [cm];y [cm];z
    // [cm]",1,xmin,xmax,1,-yhigh,yhigh,1,zmin,zmax); scale->Draw(""); gStyle->SetOptStat(0);
    ...
    std::cout << "n_failed_fits: " << m_tracks.size() << "\n";
    // c1->Write();
  }
