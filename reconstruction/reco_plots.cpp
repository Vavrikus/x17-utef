...
namespace
{
  ...
  {
    if (use_reco && p == 4)
      return electron ? -0.143346 : -0.167961;
  ...
  {
    if (use_reco && p == 4)
      return electron ? -0.0841256 : 0.0922397;
  ...
  {
    if (use_reco && p == 4)
      return electron ? 0.0208461 : -0.0331023;
  ...
  {
    if (use_reco && p == 4)
      return electron ? -0.00137187 : 0.000224353;
...
int main()
{
  ...
  TH3F* he_all = new TH3F("he_all", h_all_title, angle_bins, phi_min, phi_max, angle_bins, theta_min, theta_max,
                          energy_bins, E_min, E_max);
  TH2F* he_theta_phi
    = new TH2F("he_theta_phi", h_theta_phi_title, angle_bins, phi_min, phi_max, angle_bins, theta_min, theta_max);
  TH2F* he_theta_energy
    = new TH2F("he_theta_energy", h_theta_energy_title, angle_bins, theta_min, theta_max, energy_bins, E_min, E_max);
  TH2F* he_phi_energy
    = new TH2F("he_phi_energy", h_phi_energy_title, angle_bins, phi_min, phi_max, energy_bins, E_min, E_max);
  TH2F* he_theta_phi_abs
    = new TH2F("he_theta_phi_abs", h_theta_phi_title, angle_bins, phi_min, phi_max, angle_bins, theta_min, theta_max);
  TH2F* he_theta_energy_abs = new TH2F("he_theta_energy_abs", h_theta_energy_title, angle_bins, theta_min, theta_max,
                                       energy_bins, E_min, E_max);
  TH2F* he_phi_energy_abs
    = new TH2F("he_phi_energy_abs", h_phi_energy_title, angle_bins, phi_min, phi_max, energy_bins, E_min, E_max);

  TH3F* hp_all = new TH3F("hp_all", h_all_title, angle_bins, phi_min, phi_max, angle_bins, theta_min, theta_max,
                          energy_bins, E_min, E_max);
  TH2F* hp_theta_phi
    = new TH2F("hp_theta_phi", h_theta_phi_title, angle_bins, phi_min, phi_max, angle_bins, theta_min, theta_max);
  TH2F* hp_theta_energy
    = new TH2F("hp_theta_energy", h_theta_energy_title, angle_bins, theta_min, theta_max, energy_bins, E_min, E_max);
  TH2F* hp_phi_energy
    = new TH2F("hp_phi_energy", h_phi_energy_title, angle_bins, phi_min, phi_max, energy_bins, E_min, E_max);
  TH2F* hp_theta_phi_abs
    = new TH2F("hp_theta_phi_abs", h_theta_phi_title, angle_bins, phi_min, phi_max, angle_bins, theta_min, theta_max);
  TH2F* hp_theta_energy_abs = new TH2F("hp_theta_energy_abs", h_theta_energy_title, angle_bins, theta_min, theta_max,
                                       energy_bins, E_min, E_max);
  TH2F* hp_phi_energy_abs
    = new TH2F("hp_phi_energy_abs", h_phi_energy_title, angle_bins, phi_min, phi_max, energy_bins, E_min, E_max);
  ...
  struct AverageCounter
  {
    ...
    [[nodiscard]] double Average() const { return (numbers == 0) ? 0 : value / numbers; }
  };
  ...
  for (int i = 0; i < tracks_info->GetEntries(); i++)
  {
    ...
    // std::cout << "e_sim: " << e_sim << " theta: " << theta << " phi: " << phi << " rk_resolution: " << rk_resolution
    // << " fit_mean: " << fit_mean << std::endl;
  ...
  int e_res_bins_energy = static_cast<int>(e_res_bins / std::pow(energy_bins, 1. / 3.));
  int p_res_bins_energy = static_cast<int>(p_res_bins / std::pow(energy_bins, 1. / 3.));
  int e_res_bins_angle  = static_cast<int>(e_res_bins / std::pow(angle_bins, 1. / 3.));
  int p_res_bins_angle  = static_cast<int>(p_res_bins / std::pow(angle_bins, 1. / 3.));

  // Remaining histograms
  TH2F* he_deltaenergy_energy = new TH2F("he_deltaenergy_energy", h_deltaenergy_energy_title, energy_bins, E_min, E_max,
                                         e_res_bins_energy, res_min, res_max);
  TH2F* he_deltaenergy_energy2 = new TH2F("he_deltaenergy_energy2", h_deltaenergy_energy_title2, energy_bins + 1,
                                          E_min - 1, E_max, e_res_bins_energy, res_min, res_max);
  TH2F* he_deltaenergy_phi     = new TH2F("he_deltaenergy_phi", h_deltaenergy_phi_title, angle_bins, phi_min, phi_max,
                                          e_res_bins_angle, res_min, res_max);
  TH2F* he_deltaenergy_theta   = new TH2F("he_deltaenergy_theta", h_deltaenergy_theta_title, angle_bins, theta_min,
                                          theta_max, e_res_bins_angle, res_min, res_max);
  TH1F* he_delta_energy        = new TH1F("he_delta_energy", h_delta_energy_title, e_res_bins, res_min, res_max);

  TH2F* hp_deltaenergy_energy = new TH2F("hp_deltaenergy_energy", h_deltaenergy_energy_title, energy_bins, E_min, E_max,
                                         p_res_bins_energy, res_min, res_max);
  TH2F* hp_deltaenergy_energy2 = new TH2F("hp_deltaenergy_energy2", h_deltaenergy_energy_title2, energy_bins + 1,
                                          E_min - 1, E_max, p_res_bins_energy, res_min, res_max);
  TH2F* hp_deltaenergy_phi     = new TH2F("hp_deltaenergy_phi", h_deltaenergy_phi_title, angle_bins, phi_min, phi_max,
                                          p_res_bins_angle, res_min, res_max);
  TH2F* hp_deltaenergy_theta   = new TH2F("hp_deltaenergy_theta", h_deltaenergy_theta_title, angle_bins, theta_min,
                                          theta_max, p_res_bins_angle, res_min, res_max);
  TH1F* hp_delta_energy        = new TH1F("hp_delta_energy", h_delta_energy_title, p_res_bins, res_min, res_max);

  for (TH2F* h : {
         he_deltaenergy_energy,
         he_deltaenergy_energy2,
         he_deltaenergy_phi,
         he_deltaenergy_theta,
         hp_deltaenergy_energy,
         hp_deltaenergy_energy2,
         hp_deltaenergy_phi,
         hp_deltaenergy_theta,
       })
  {
    ApplyThesisStyle(h);
  ...
  auto th2_c = [](TCanvas* c)
  {
    ...
    float e_mar_r = 0.035;
    float e_mar_b = 0.03;
    float wf      = 0.9F / (0.9F - e_mar_l - e_mar_r);
    float hf      = 0.9F / (0.9F - e_mar_b);
    c->SetLeftMargin(e_mar_l + 0.1F);
    c->SetRightMargin(e_mar_r + 0.1F);
    c->SetBottomMargin(e_mar_b + 0.1F);
    c->SetWindowSize(static_cast<int>(wf * 700), static_cast<int>(hf * 500));
  };
  ...
  for (int i = 0; i < 2; i++)
  {
    ...
    h_all[i]->SetStats(false);
    h_all[i]->SetContour(ncont);
    h_all[i]->Draw("box2 z");
    c->Write();
  }

  std::string c_names_extras[12] = {
    "c_e_theta_phi",      "c_e_theta_energy",  "c_e_phi_energy",       "c_p_theta_phi",
    "c_p_theta_energy",   "c_p_phi_energy",    "c_e_theta_phi_abs",    "c_e_theta_energy_abs",
    "c_e_phi_energy_abs", "c_p_theta_phi_abs", "c_p_theta_energy_abs", "c_p_phi_energy_abs",
  };
  TH2F* h_extras[12] = {
    he_theta_phi,     he_theta_energy,     he_phi_energy,     hp_theta_phi,     hp_theta_energy,     hp_phi_energy,
    he_theta_phi_abs, he_theta_energy_abs, he_phi_energy_abs, hp_theta_phi_abs, hp_theta_energy_abs, hp_phi_energy_abs,
  };
  ...
  for (int i = 0; i < 12; i++)
  {
    ...
    gStyle->SetPalette(kLightTemperature);
    h_extras[i]->SetStats(false);
    h_extras[i]->SetContour(ncont);
    h_extras[i]->Draw("colz");
    c->Write();
  }

  std::string c_names_2d[8] = {
    "c_e_deltaenergy_energy", "c_e_deltaenergy_energy2", "c_e_deltaenergy_theta", "c_e_deltaenergy_phi",
    "c_p_deltaenergy_energy", "c_p_deltaenergy_energy2", "c_p_deltaenergy_theta", "c_p_deltaenergy_phi",
  };
  TH2F* histos_2d[8] = {
    he_deltaenergy_energy, he_deltaenergy_energy2, he_deltaenergy_theta, he_deltaenergy_phi,
