...
public:
  ...
  void ElectronLoop() override
  {
    ...
    X17::RecoPoint reco   = X17::Reconstruct(loop->map, micro);
    ...
    m_v_res_x.push_back(reco.x() - micro.x0());
    m_v_res_y.push_back(reco.y() - micro.y0());
    m_v_res_z.push_back(reco.z() - micro.z0());
    m_v_res_total.push_back(
  ...
  void PostTrackLoop() override
  {
    ...
    c->Divide(2, 2);
    int xbins = GetBinsFreedmanDiaconis(m_v_res_x, -0.4, 0.4);
    int ybins = GetBinsFreedmanDiaconis(m_v_res_y, -0.4, 0.4);
    int zbins = GetBinsFreedmanDiaconis(m_v_res_z, -0.4, 0.4);
    int rbins = GetBinsFreedmanDiaconis(m_v_res_total, 0, 0.4);
...
public:
  MapRecoTask(std::string name, bool electrons, bool positrons)
