...
int main()
{
  ...
  TTree* tracks = static_cast<TTree*>(input->Get("tracks_full"));
  ...
  size_t n_driftlines = track->driftlines.size();
  std::cout << "Loaded track with " << track->points.size() << " points and " << n_driftlines << " driftlines.\n";
  size_t drift_pts = 0;
  for (const auto& driftline : track->driftlines)
    drift_pts += driftline.size();
  std::cout << "Loaded " << drift_pts << " drift points (average "
            << static_cast<double>(drift_pts) / static_cast<double>(n_driftlines) << " per driftline).\n";
  ...
  for (const auto& driftline : track->driftlines)
  {
    ...
    l_driftlines_3d.push_back(new TPolyLine3D(static_cast<int>(driftline.size())));
    ...
    {
      g->SetLineColor(static_cast<Color_t>(X17::Color::GOrange()));
      g->SetLineWidth(1);
      ApplyThesisStyle(g);
    }

    l_driftlines_3d.back()->SetLineColor(static_cast<Color_t>(X17::Color::GOrange()));
  ...
  for (TPolyLine3D* l : l_driftlines_3d)
    l->Draw("same");
  c_xyz->Write();
