  ...
  void watchAndDisplay(const char* filename, const char* canvasName)
  {
    ...
    while (true)
    {
      ...
      if (newMTime != lastMTime)
      {
        ...
        TCanvas* fileCanvas = static_cast<TCanvas*>(f->Get(canvasName));
        if (fileCanvas)
        {
          // CRITICAL: Delete old canvas to close window/free memory
          delete displayedCanvas;

          // CRITICAL: Clone the canvas so it survives after file close
          displayedCanvas = static_cast<TCanvas*>(fileCanvas->Clone());
...
// --- Menu Logic ---
void cview_update() // NOLINT(misc-use-internal-linkage)
{
  ...
  while (true) // OUTER LOOP: Select File
  {
    ...
    if (filename == "exit" || filename.empty())
      break;
