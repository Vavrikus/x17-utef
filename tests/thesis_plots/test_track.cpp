...
namespace
{
  ...
  X17::TrackMicro LoadTrack(const std::string& folder, bool newfile = false)
  {
    std::string track_path = "../../../data/single_track/" + folder + "/electrons.root";
    TFile* track_file      = new TFile(track_path.c_str());
    TTree* track_tree      = static_cast<TTree*>(track_file->Get("electrons"));
  ...
  void PlotDriftXZ(const X17::TrackMicro& track, bool newcoords = false)
  {
    ...
    {
      line->SetLineColor(static_cast<Color_t>(X17::Color::GOrange()));
      line->SetLineWidth(2);
      line->Draw();
    }
  ...
  void PlotDriftXY(const X17::TrackMicro& track, const std::string& filename, bool newcoords = false)
  {
    ...
    im_xy->Scale(iround(1.00 * g_cwidth * (1 - l - r) * 15 / (g_xmax - g_xmin)),
                 iround((1414. / 1478.) * g_cheight * (1 - b - t) * 0.6 / (g_ymax - g_ymin)));
    im_xy->Draw();
  ...
  void PlotDriftYZ(const X17::TrackMicro& track, const std::string& filename, bool newcoords = false)
  {
    ...
    // weird factor necessary to get correct size
    im_yz->Scale(iround(1.004 * g_cwidth * (1 - l - r) * 0.6 / (g_ymax - g_ymin)),
                 iround(0.957 * g_cheight * (1 - b - t) * 11 / (g_zmax - g_zmin)));
    im_yz->Draw();
  ...
  void PlotTrackRK(const X17::TrackMicro& track, X17::Field<X17::Vector>* magfield, bool newcoords = false)
  {
    double Ekin = std::sqrt(64E+12 + X17::constants::E0 * X17::constants::E0) - X17::constants::E0;
    X17::RK4<8>* trackrk
      = X17::GetTrackRK(*magfield, true, 1E-13 * 16.66, Ekin, X17::Vector(0, 0, 0), X17::Vector(1, 0, 0), true);
    trackrk->Integrate();
    std::vector<X17::RKPoint> rk_points;
    for (const auto& vec : trackrk->GetResults())
    {
      using namespace X17::constants;
      rk_points.emplace_back(m2cm * vec.at(1, 0), m2cm * vec.at(2, 0), m2cm * vec.at(3, 0), 1e+9 / c * vec.at(0, 0));
    }
    ...
    for (const auto& point : track.points)
    {
      ...
      X17::Vector start_new = newcoords ? X17::Vector{ start_old.x, start_old.y, start_old.z }
                                        : X17::Vector{ start_old.z, start_old.x, start_old.y };
  ...
  void PlotRASD(const X17::TrackMicro& track, X17::DriftMap* map, bool newcoords = false)
  {
    ...
    for (const auto& point : track.points)
    {
      ...
      if (z_end > 7.5)
      {
        ...
        if (orig.x > x_limit && orig.x < map->GetXMax() && reco_map.x() != 0)
        {
          ...
          max_residual         = std::max(residual.Magnitude(), max_residual);
          residuals.push_back(residual);

          X17::Vector oldvnew = reco_map_old.AsVector() - reco_map.AsVector();
          max_oldvsnew        = std::max(oldvnew.Magnitude(), max_oldvsnew);
    ...
    TH1F* h_xres = new TH1F("", ";x residual [cm];# of electrons",
                            GetBinsScott(hmin, hmax, sigma.x, static_cast<int>(residuals.size())), hmin, hmax);
    TH1F* h_yres = new TH1F("", ";y residual [cm];# of electrons",
                            GetBinsScott(hmin, hmax, sigma.y, static_cast<int>(residuals.size())), hmin, hmax);
    TH1F* h_zres = new TH1F("", ";z residual [cm];# of electrons",
                            GetBinsScott(hmin, hmax, sigma.z, static_cast<int>(residuals.size())), hmin, hmax);
    TH1F* h_res  = new TH1F("", ";residual [cm];# of electrons",
                            GetBinsScott(0, hmax, mag_sigma, static_cast<int>(residuals.size())), 0, hmax);
    ...
    TH1F* h_yres2 = new TH1F("", "Y residuals;#Delta y [cm];# of electrons",
                             GetBinsScott(hmin2, hmax2, sigma2.y, static_cast<int>(oldvsnew.size())), hmin2, hmax2);
    TH1F* h_xres2 = new TH1F("", "X residuals;#Delta x [cm];# of electrons",
                             GetBinsScott(hmin2, hmax2, sigma2.x, static_cast<int>(oldvsnew.size())), hmin2, hmax2);
    TH1F* h_zres2 = new TH1F("", "Z residuals;#Delta z [cm];# of electrons",
                             GetBinsScott(hmin2, hmax2, sigma2.z, static_cast<int>(oldvsnew.size())), hmin2, hmax2);
    TH1F* h_res2  = new TH1F("", "Residuals;Deviation [cm];# of electrons",
                             GetBinsScott(0, hmax2, mag_sigma2, static_cast<int>(oldvsnew.size())), 0, hmax2);
  ...
  void PlotPadReco(const X17::DriftMap& map, int pad_id, bool m7030 = true, bool old_reco = false)
  {
    ...
    double time_step       = 10;
    int i_max              = ifloor(max_time / time_step);
    double time_step2      = 100;
    int plane_steps        = 20;
    int i_max2             = ifloor(max_time / time_step2);
    ...
    for (auto v_corner : corners)
    {
      ...
      for (int i = 0; i < i_max; i++)
      {
        ...
        X17::RecoPoint reco
          = old_reco ? X17::ReconstructOld(map, t_corner, 1e-6, !m7030) : X17::Reconstruct(map, t_corner);
        corner_line->SetPoint(i, reco.x(), reco.y(), reco.z());

        x_min = std::min(reco.x(), x_min);
        x_max = std::max(reco.x(), x_max);
        y_min = std::min(reco.y(), y_min);
        y_max = std::max(reco.y(), y_max);
      }
    ...
    for (int i = 0; i < i_max2; i++)
    {
      ...
      X17::RecoPoint creco = old_reco ? X17::ReconstructOld(map, xc, yc, time + time_step2 / 2., 1e-6, !m7030)
                                      : X17::Reconstruct(map, { xc, yc, -8, time + time_step2 / 2. }, g_map_pts);
  ...
  std::pair<TGraph*, TGraph*> MapRecoPoints(const X17::TrackMicro& track, X17::DriftMap* map, bool newcoords = false)
  {
    TGraph* xz_original = new TGraph();
    TGraph* xz_reco_map = new TGraph();
    double x_limit      = 2; // newcoords ? -1 : 2;
  ...
  void PlotSpline(const X17::TrackMicro& track, X17::Field<X17::Vector>* magfield, X17::DriftMap* map,
                  bool newcoords = false)
  {
    double x_limit                  = 2;
    auto [xz_original, xz_reco_map] = MapRecoPoints(track, map, newcoords);
    ...
    for (double x = x_limit; x < map->GetXMax();
         x += (map->GetXMax() - x_limit) / xsteps) // NOLINT(bugprone-float-loop-counter)
    {
  ...
  void PlotCircle2D(const X17::TrackMicro& track, X17::Field<X17::Vector>* magfield, X17::DriftMap* map,
                    bool newcoords = false)
  {
    double x_limit                  = 2;
    auto [xz_original, xz_reco_map] = MapRecoPoints(track, map, newcoords);
...
int main(int argc, char* argv[])
{
  ...
  // X17::RKFit fit = X17::RKFit(magfield, true, 1E-13*16.66, X17::Vector(0,0,0), X17::Vector(1,0,0), reco_points,
  // true); fit.SetFitter(); fit.SetEnergy(cfit3d_E_mid); fit.FitRK(); fit.PrintFitParams();
