...
int main(int argc, char* argv[])
{
  ...
  TH2F* frame
    = new TH2F("frame", ";x [cm];y [cm];B [T]", 10, -padding, xmax + padding, 10, -ymax - padding, ymax + padding);
  ...
  for (double x = 0; x <= xmax; x += spacing) // NOLINT(bugprone-float-loop-counter)
  {
    for (double y = -ymax; y <= ymax; y += spacing) // NOLINT(bugprone-float-loop-counter)
    {
      try
      {
        ...
        {
          int colorIndex = gStyle->GetColorPalette(ifloor(7.5 * b.Magnitude() * 100) % 255);

          TArrow* arrow
            = new TArrow(x, y, x + scale * b.x, y + scale * b.y, static_cast<float>(b.Magnitude()) / 12, "|>");
          arrow->SetLineColor(static_cast<Color_t>(colorIndex));
          arrow->SetFillColor(static_cast<Color_t>(colorIndex));
          arrow->Draw();
        }
      }
      catch (...)
      {
        std::cerr << "Failed to get field at point (" << x << ", " << y << ")\n";
      }
    }
  }
  ...
  for (int i = 0; i < 1000; i++)
  {
    for (int j = 0; j < 1000; j++)
    {
      ...
      try
      {
        b = magfield->GetField(x, 0, z);
      }
      catch (...)
      {
        std::cerr << "Failed to get field at x = " << x << ", z = " << z << "\n";
      }
