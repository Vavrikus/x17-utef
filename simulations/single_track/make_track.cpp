...
int main(int argc, char* argv[])
{
  ...
  constexpr double ekin = 8.e6;  // Set the particle kinetic energy [eV].
  ...
  while (track.GetCluster(xc, yc, zc, tc, nc, ec, extra))
  {
    ...
    for (int j = 0; j < nc; ++j)
    {
      ...
      aval.GetElectronEndpoint(0, point.start.point.x, point.start.point.y, point.start.point.z, point.start.t,
                               point.e0, point.end.point.x, point.end.point.y, point.end.point.z, point.end.t, point.e1,
                               status);
      electrons.Fill();
      k++;
    }
  ...
  std::cout << "Total electron generated: " << k << '\n';
