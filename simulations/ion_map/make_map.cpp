...
namespace
{
  ...
  TChain* LoadData(int max_id, const std::string& folder = "../../../data/ion_map/sample_2.0/")
  {
  ...
  TGraph* GetQQconfidenceGraph(double sigma, std::vector<std::vector<double>> sq_mahal_sets, TGraph* g_rand,
                               bool no_z = true)
  {
    int nsets              = static_cast<int>(sq_mahal_sets.size());
    int rand_size          = static_cast<int>(sq_mahal_sets.back().size());
    int degrees_of_freedom = no_z ? 3 : 4;

    TGraph* g_rand_ci = new TGraph(2 * rand_size);
    double confidence = TMath::Erf(sigma / std::numbers::sqrt2);
  ...
  int make_map()
  {
    ...
    X17::DriftMap map({ map_xmin, -30, -8 }, { 15, 30, 8 }, step_size, X17::MapPoint());
    ...
    double Bmax = 7.5;
    // Scott's rule
    int Bbins           = GetBinsScott(Bmin, Bmax, 1.0, 651 * 16);
    int Bbins_rand      = GetBinsScott(Bmin, Bmax, 1.0, nsets);
    ...
    int Abins            = iround((Amax - Amin) / (2 * 5.8 / std::pow(651 * 16, 1.0 / 3.0)));
    int Abins_rand       = iround((Amax - Amin) / (2 * 5.8 / std::pow(nsets, 1.0 / 3.0)));
    ...
    TH1F* h_pvalsA = new TH1F("h_pvalsA", "p-values", iround(std::pow(651 * 16, 1.0 / 3.0)), 0, 1);
    TH1F* h_pvalsB = new TH1F("h_pvalsB", "p-values", iround(std::pow(651 * 16, 1.0 / 3.0)), 0, 1);
    ...
    for (int i = 0; i < map_data_in->GetEntries(); i++)
    {
      ...
      ReportProgress(i + 1, static_cast<int>(map_data_in->GetEntries()));
      ...
      else
      {
        ...
        if (i != 0)
        {
          map.SetPoint(v_prev, X17::MapPoint(static_cast<int>(p_vec.size()), p_avg, CovarianceMatrix(p_vec, p_avg)));
          ...
          if (p_vec.size() < 100)
            std::cout << "WARNING: Map point has only " << p_vec.size() << " entries!\n";
          p_vec.clear();
        }
    ...
    outfile->WriteObjectAny(&map, "X17::DriftMap", "map");
    const X17::DriftMap& cmap = map; // Map should no longer be modified.
    ...
    if (MakePlots)
    {
      ...
      unsigned long rand_size = sq_mahal_sets.back().size();
      TGraph* g_rand_ci       = GetQQconfidenceGraph(3.0, sq_mahal_sets, g_rand);
      g_rand_ci->SetFillColor(static_cast<Color_t>(X17::Color::RGB(255, 0, 0, 15)));
      ...
      g_rand_ci2->SetFillColor(static_cast<Color_t>(X17::Color::RGB(255, 0, 0, 35)));
      ...
      TF1* chi2_pdf = new TF1(
        "chi2_pdf",
        "[0] * TMath::Power(x, [1]/2 - 1) * TMath::Exp(-x/2) / (TMath::Gamma([1]/2) * TMath::Power(2, [1]/2))", 0, 30);
      ...
      int y_xzi   = iround(y_xz - map.GetYMin() / map.GetStep()); // The y-coordinate index of xz plots.
...
int main()
{
  ...
  {
    std::cerr << "Mapping Error (Out of Range): " << e.what() << '\n';
    return 1;
  }
