...
#include "TEllipse.h"
#include "TMath.h"
#include "TMatrixDBase.h" // IWYU pragma: keep
...
namespace X17
{
  void MapPoint::Diagonalize(bool no_z)
  {
    ...
    TMatrixDSym cov(4); // NOLINT(misc-include-cleaner)
    ...
    if (!cov.TMatrixDBase::IsSymmetric()) // NOLINT(misc-include-cleaner)
  ...
  TEllipse* MapPoint::GetErrorEllipse(int skip_index, double sigma, bool x_bigger) const
  {
    ...
    double theta = sign(v1.y) * v1.Angle({ 1, 0, 0 }) * 180 / std::numbers::pi;

    return new TEllipse((*this)[min_index], (*this)[max_index], std::sqrt(lambda_1 * factor),
                        std::sqrt(lambda_2 * factor), 0, 360, theta);
  }
