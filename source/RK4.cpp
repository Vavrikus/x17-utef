...
namespace X17
{
  ...
  Matrix<8, 1> GetInitParams(double kin_en, Vector origin, Vector orientation)
  {
    ...
    return Matrix<8, 1>({ 0, start.x, start.y, start.z, gamma * c, gamma * velocity * orientation.x,
                          gamma * velocity * orientation.y,
                          gamma * velocity * orientation.z }); // NOLINT(readability-trailing-comma)
  }
  ...
  Matrix<4, 4> GetEMtensor(const Field<Vector>& magfield, Vector position)
  {
    ...
    return Matrix<4, 4>({ 
      ...
      m2cm * efield.z / c,  b.y,                 -b.x,                  0    // NOLINT(readability-trailing-comma)
    });
    // clang-format on
  }
                ...
                Matrix<8, 1>& output)
  {
    ...
    output = Matrix<8, 1>({ params.at(4, 0), params.at(5, 0), params.at(6, 0), params.at(7, 0), out1.at(0, 0),
                            out1.at(1, 0), out1.at(2, 0), out1.at(3, 0) }); // NOLINT(readability-trailing-comma)
  }

  bool IsOutOfTPC(double /*tau*/, const Matrix<8, 1>& params)
  {
    using namespace constants;
    return (!IsInTPC(m2cm * params.at(1, 0), m2cm * params.at(2, 0), m2cm * params.at(3, 0),
                     -0.5)); // && (m2cm * params.at(1,0) > xmin);
  }
                     ...
                     Vector orientation, bool big_volume)
  {
    ...
    if (big_volume)
      return new RK4<8>(0, step, f, init, IsOutOfTPCandRect);
    return new RK4<8>(0, step, f, init, IsOutOfTPC);
  }
  ...
  RKPoint GetTrackRKPoint(const RK4<8>* track, int index)
  {
    ...
    return RKPoint(m2cm * rk_pts[index].at(1, 0), m2cm * rk_pts[index].at(2, 0), m2cm * rk_pts[index].at(3, 0),
                   1e+9 / c * rk_pts[index].at(0, 0));
  }
  ...
  double GetTrackRKSqDist(const RK4<8>* track, Vector point)
  {
    ...
    if (max_index <= min_index)
      throw std::runtime_error("GetTrackRKSqDist: Runge-Kutta trajectory has less than three points.");
  ...
  TGraph2D* GetGraphRK(const RK4<8>* rk_track)
  {
    ...
    for (auto r : results)
    {
      ...
      if (IsInTPC(point))
        output->AddPoint(point.x, point.y, point.z);
    }
    return output;
  }
  ...
  RKFit::RKFit(Field<Vector>* magfield, bool electron, double step, Vector origin, Vector orientation,
               const std::vector<RecoPoint>& fit_data, bool big_volume)
    : m_magfield(magfield),
      m_electron(electron),
      m_step(step),
      m_origin(origin),
  ...
  void RKFit::SetFitter(int parameters, bool print)
  {
    ...
    m_fitter->SetFCN(X17::RKFit::Eval_);
  }
  ...
  void RKFit::SumSqDist_(int& /*npar*/, double* /*gin*/, double& sumsq,
                         double* par, // NOLINT(readability-non-const-parameter)
                         int /*iflag*/)
  {
    m_kin_en = par[0];
    ...
    for (auto p : m_fit_data)
      sumsq += p.count * GetTrackRKSqDist(m_curr_rk, p.AsVector());
  }
} // namespace X17