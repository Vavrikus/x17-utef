...
namespace X17
{
  ...
  std::array<std::array<double, 8>, 3> GetInterpolCoef(const Field<MapPoint>& map, const int (&indices)[6])
  {
    ...
    std::array<std::array<double, 8>, 3> xytvalues; // An array of size 3, containing arrays of the x, y, and t values
                                                    // at each corner of the pseudocube used for interpolation.
    ...
    for (int i = 0; i < 3; i++)
    {
      ...
      {
        arr[9ULL * j]     = 1;
        arr[9ULL * j + 1] = xytvalues[0][j];
        arr[9ULL * j + 2] = xytvalues[1][j];
        arr[9ULL * j + 3] = xytvalues[2][j];
        arr[9ULL * j + 4] = xytvalues[0][j] * xytvalues[1][j];
        arr[9ULL * j + 5] = xytvalues[0][j] * xytvalues[2][j];
        arr[9ULL * j + 6] = xytvalues[1][j] * xytvalues[2][j];
        arr[9ULL * j + 7] = xytvalues[0][j] * xytvalues[1][j] * xytvalues[2][j];

        const auto corner = CubeCorner(bounds, j);
        arr[9ULL * j + 8] = corner[i];
      }
  ...
  void FindMapMinMaxIndex(const Field<MapPoint>& map, int& i_min, int& i_max, int (&i_mid)[3], int var, double value)
  {
    ...
    if (!should_continue())
      std::cerr << "WARNING: Binary search (" << var << ") skipped.\n";
#endif
    ...
    while (should_continue())
    {
      ...
      if (mid < low || mid > high)
        std::cerr << "WARNING: Ordering mistake in binary search.\n";
#endif
    }
  ...
  {
    int status;
    if (CellContainsReco(map, indices, end_point, status))
      return;
    ...
    for (int i = 0; i < 8; i++)
    {
      ...
      std::cout << " (x,y,t) = (" << map.at(xi, yi, zi).point.x() << ", " << map.at(xi, yi, zi).point.y() << ", "
                << map.at(xi, yi, zi).point.t << ")\n";
    }
    std::cout << "-------------------------------------------------------------------------------\n\n";
  }
  ...
  RecoPoint Reconstruct(const Field<MapPoint>& map, EndPoint end_point, TGraph2D* g_map_pts)
  {
    ...
    do
    {
      ...
      if (i_mid[0] != 0)
      {
        while (map.at(i_mid[0], yimin, i_mid[2]).point.y() == 0)
          yimin++;
        while (map.at(i_mid[0], yimax, i_mid[2]).point.y() == 0)
          yimax--;
      }
    ...
    while (status != -1)
    {
      ...
      int var      = status / 2;
      bool err_max = (status % 2) != 0;

      bool var_is_z = (var == 2);
      if (var_is_z)
        err_max = !err_max;

      if (err_max && boundaries[2 * var + static_cast<int>(!var_is_z)] < cells[var] - 1)
      {
        // std::cout << "Boundaries increased\n";
        boundaries[2ULL * var]++;
        boundaries[2ULL * var + 1]++;
      }
      else if (!err_max && boundaries[2 * var + static_cast<int>(var_is_z)] > 0)
      {
    ...
    if (g_map_pts)
      for (int i = 0; i < 8; i++)
      {
        ...
        g_map_pts->AddPoint(map.GetXMin() + xi * map_step, map.GetYMin() + yi * map_step,
                            map.GetZMin() + zi * map_step);
      }

    // Interpolation from map.
    std::array<std::array<double, 8>, 3> coef = GetInterpolCoef(map, boundaries);

    double xout = coef[0][0] + coef[0][1] * x1 + coef[0][2] * y1 + coef[0][3] * t1 + coef[0][4] * x1 * y1
                  + coef[0][5] * x1 * t1 + coef[0][6] * y1 * t1 + coef[0][7] * x1 * y1 * t1;
    double yout = coef[1][0] + coef[1][1] * x1 + coef[1][2] * y1 + coef[1][3] * t1 + coef[1][4] * x1 * y1
                  + coef[1][5] * x1 * t1 + coef[1][6] * y1 * t1 + coef[1][7] * x1 * y1 * t1;
    double zout = coef[2][0] + coef[2][1] * x1 + coef[2][2] * y1 + coef[2][3] * t1 + coef[2][4] * x1 * y1
                  + coef[2][5] * x1 * t1 + coef[2][6] * y1 * t1 + coef[2][7] * x1 * y1 * t1;

    double x, y, z;
    // std::cout << "Min corner:          (" << map.GetXMin()+ximin*map.GetStep() << ", " <<
    // map.GetYMin()+yimin*map.GetStep() << ", " << map.GetZMin()+zi_tmax*map.GetStep() << ")\n"; std::cout <<
    // "Reconstructed point: (" << xout << ", " << yout << ", " << zout << ")\n";
    return RecoPoint(xout, yout, zout, 1);
  }

  double Offset(const MapPoint& p, double x1, double y1, double t1, double tfact)
  {
  ...
  RecoPoint ReconstructOld(const Field<MapPoint>& map, double x1, double y1, double t1, double max_err, bool gas9010)
  {
    ...
    do
    {
      ...
      // Check bounds.
      x = std::max(x, map.GetXMin());
      x = std::min(x, map.GetXMax());
      y = std::max(y, map.GetYMin());
      y = std::min(y, map.GetYMax());
      z = std::max(z, map.GetZMin());
      z = std::min(z, map.GetZMax());
      ...
      x = std::max(x, map.GetXMin() + step);
      x = std::min(x, map.GetXMax() - step);
      y = std::max(y, map.GetYMin() + step);
      y = std::min(y, map.GetYMax() - step);
      z = std::max(z, map.GetZMin() + step);
      z = std::min(z, map.GetZMax() - step);

      double damped_grad_size = damp * std::sqrt(gradx * gradx + grady * grady + gradz * gradz);
      if (damped_grad_size > 0.1 * offset)
        damp /= 10;
