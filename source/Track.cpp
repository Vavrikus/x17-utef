...
// ClassImp(X17::TrackRK) ClassImp(X17::TrackMicro) ClassImp(X17::TrackInfo)
...
namespace X17
{
  void GetRandomTrackParams(TRandom3* rand, bool& electron, Vector& origin, Vector& orient, double& e_kin)
  {
    ...
    double phi = RandomMinMax(rand, 0, 2 * std::numbers::pi); // The azimuth of the initial point on the circle target.
    double r
      = std::sqrt(RandomMinMax(rand, 0, r_max * r_max)); // The distance of the initial point from the circle target.
