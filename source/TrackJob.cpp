...
namespace X17
{
  ...
  void TrackJob::SetParameters(int argc, char* argv[])
  {
    ...
    else
    {
      ...
      if (argc < 6)
      {
        std::cerr << "ERROR: Missing arguments in ion_electrons. Correct arguments: max_id, id, iterations, "
                     "angle_bins, energy_bins.\n";
      }
      ...
      if (angle_bins <= 1)
        std::cerr << "ERROR: Parameter angle_bins has to be greater than 1.\n";
      if (energy_bins <= 1)
        std::cerr << "ERROR: Parameter energy_bins has to be greater than 1.\n";
      ...
      std::cout << "Number of unique tracks in all jobs: " << n_sets << ", in this job tracks from " << min_set
                << " to " << max_set;
      std::cout << " will be simulated.\n\n";
    }
  }

  void TrackJob::GetTrackParameters(int n_set, bool& electron, Vector& origin, Vector& orient, double& e_kin) const
  {
    // Getting bin info from the value of n_set.
    electron     = (((n_set - 1) % 2) != 0);
