...
namespace X17
{
  ...
  double DefaultLayout::GetPadHeight(int i, bool effective) const
  {
    ...
    if (effective)
      std::cerr << "Warning (GetPadHeight): effective not implemented.";
  ...
  void DefaultLayout::GetPadInfo(int i, int& out_column, int& out_row, double& out_height) const
  {
    using namespace constants;
    if (i < 1 || i > channels)
      std::cerr << "WARNING: Invalid channel number " << i << " (must be between 1 and " << channels << ").\n";
    ...
    int part12_channels
      = part1_channels + triangle(columns - 1) - triangle(columns - 1 - triangle_row2 + triangle_row1);

    // Part with same length rows (first six rows).
    if (i <= part1_channels)
    {
      out_row    = (i - 1) / columns + 1;
      out_column = i % columns;
    }
    ...
    else
    {
      ...
      out_column      = (i - part12_channels) - triangle(columns - 2 - triangle_row2 + triangle_row1)
                        + triangle(max_cols_p3 + triangle_row2 - out_row);
    }

    if (out_column == 0)
      out_column = 12; // Number 0 corresponds to 12th column.
  ...
  void DefaultLayout::GetPadCorners(int i, double& out_xlow, double& out_ylow, double& out_xhigh, double& out_yhigh,
                                    bool nogaps) const
  {
    using namespace constants;
    ...
    if (nogaps)
      gaps_correction = pad_offset / 2.0;
    ...
    out_ylow
      = -(yhigh + pad1_offset - (row - 1) * (pad_height + pad_offset) - (column - 1) * pad_stag + gaps_correction);
  ...
  void DefaultLayout::DrawPads(bool nogaps, bool inverted, TCanvas* c) const
  {
    ...
    if (c)
      c = new TCanvas("c_pads", "GEM readout pads", 600, iround(600 * 2 * (yhigh + 1) / (xmax - xmin + 2)));
    ...
    for (auto* l : pad_lines)
      l->Draw("AL");
    for (auto* t : pad_numbers)
      t->Draw("same");

    DrawTrapezoid(static_cast<short>(inverted));
  }
  ...
  void DefaultLayout::DrawPadsDistortion(double time, TCanvas* c, const Field<MapPoint>* map) const
  {
    ...
    if (map)
    {
      ...
      map            = reinterpret_cast<DriftMap*>(inFile2->Get("map"));
    }
    ...
    if (c)
    {
      c = new TCanvas("c_pads_distorted", c_title.c_str(), iround(600 * 2 * (yhigh + 1) / (xmax - xmin + 2)), 600);
    ...
    for (int i = 1; i <= channels; i++)
    {
      ...
      pad_lines.push_back(
        new TLine(-bottomleft.y(), bottomleft.x(), -bottomright.y(), bottomright.x()));       // Bottom line.
