...
namespace X17
{
  ...
  double circle_func(double* x, double* par) // NOLINT
  {
    const double& xx      = x[0];
    const double& radius  = par[0];
    const double& node1_x = par[1];
    const double& node2_x = par[2];
    const double& node1_y = par[3];
    const double& node2_y = par[4];
  ...
  void RecoEnergy(TF1* fit, const Field<Vector>& magfield, TGraph* magnetic, double min, double max, double step)
  {
    ...
    const double r = cm2m * fit->GetParameter(0);

    // Mean magnetic field.
    Vector B = magfield.GetField((xmax + xmin) / 2, 0, fit->Eval((xmax + xmin) / 2));

    for (double x = min; x <= max; x += step) // NOLINT(bugprone-float-loop-counter)
    {
