...
namespace X17
{
      ...
      for (int yi = 0; yi < magfield.GetYCells(); yi++)
        for (int zi = 0; zi < magfield.GetZCells(); zi++)
        {
          ...
          if (IsInTPC(x, y, z, dist))
          {
            ...
            max_magnitude_sq = std::max(v.SqMagnitude(), max_magnitude_sq);
          }
        }
      ...
      for (int yi = 0; yi < magfield.GetYCells(); yi++)
        for (int zi = 0; zi < magfield.GetZCells(); zi++)
        {
          ...
          if (IsInTPC(x, y, z, dist))
          {
            ...
            max_angle = std::max(v.Angle(efield), max_angle);
          }
        }
  ...
  void DrawTrapezoid(short width, bool yxformat)
  {
    using namespace constants;
  ...
  void DrawFirstSectorMagnets(bool yxformat)
  {
    ...
    double angle = 30.0 * std::numbers::pi / 180.0;
