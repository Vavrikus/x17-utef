...
#include "TMatrixDSymEigen.h" // IWYU pragma: keep
...
namespace X17
{
  Matrix<4, 4> CovarianceMatrix(const std::vector<EndPoint>& values, EndPoint average)
  {
    ...
    cov_mat /= static_cast<double>(vectors.size()) - 1.0;
  ...
  std::vector<double> SqMahalanobis(const std::vector<EndPoint>& values, bool no_z)
  {
    ...
    if (no_z)
    {
      TMatrixD cov_m(3, 3); // NOLINT(misc-include-cleaner)
      cov_m(0, 0) = cov.at(0, 0);
      cov_m(0, 1) = cov.at(0, 1);
      cov_m(0, 2) = cov.at(0, 3);
      cov_m(1, 0) = cov.at(1, 0);
      cov_m(1, 1) = cov.at(1, 1);
      cov_m(1, 2) = cov.at(1, 3);
      cov_m(2, 0) = cov.at(3, 0);
      cov_m(2, 1) = cov.at(3, 1);
      cov_m(2, 2) = cov.at(3, 3);

      TMatrixD cov_inv(TMatrixD::kInverted, cov_m);

      for (const EndPoint& p : values)
      {
        TVectorD tv(3); // NOLINT(misc-include-cleaner)
        tv(0) = p.x() - average.x();
        tv(1) = p.y() - average.y();
        tv(2) = p.t - average.t;
  ...
  void FillQQplot(TGraph* graph, const std::vector<EndPoint>& values, bool no_z)
  {
    ...
    std::ranges::sort(sq_mahal);

    for (int i = 0; i < sq_mahal.size(); i++)
    {
      double chi2_quantile
        = TMath::ChisquareQuantile((i + 1.0) / (static_cast<double>(sq_mahal.size()) + 1), degrees_of_freedom);
