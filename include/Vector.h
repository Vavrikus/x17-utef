...
namespace X17
{
  ...
  struct Vector
  {
    ...
    [[nodiscard]] double SqMagnitude() const { return x * x + y * y + z * z; }

    /// @brief Compute the magnitude of this vector.
    /// @return The magnitude.
    [[nodiscard]] double Magnitude() const { return std::sqrt(this->SqMagnitude()); }
    ...
    [[nodiscard]] double Angle(Vector other) const
    {
    ...
    [[nodiscard]] double SqDist(Vector other) const
    {
      double dx = x - other.x;
      double dy = y - other.y;
      double dz = z - other.z;
    ...
    /// @param[in] other The other vector.
    /// @return The distance.
    [[nodiscard]] double Dist(Vector other) const { return std::sqrt(this->SqDist(other)); }
    ...
    Vector operator+(Vector v2) const { return Vector{ x + v2.x, y + v2.y, z + v2.z }; }
    ...
    Vector operator*(double d) const { return Vector{ d * x, d * y, d * z }; }
    ...
    double operator*(Vector v2) const { return x * v2.x + y * v2.y + z * v2.z; }
  ...
  /// @brief Computes the squared distance between a line and a point in 3D space and returns the closest point on the
  /// line.
  /// @param origin The origin of the line.
  /// @param orientation The orientation vector of the line.
  /// @param max_param The maximum allowed parameter of the line.
