...
namespace X17
{
  /// @brief A struct for storing the reconstructed initial points of the track. If points are reconstructed from pads,
  /// count/charge also relevant.
  struct RecoPoint
  {
    ...
    RecoPoint(double x, double y, double z, int count)
      : point(x, y, z, -1), count(count)
    {
    }
    ...
    RecoPoint& operator=(const RecoPoint& other) = default;
