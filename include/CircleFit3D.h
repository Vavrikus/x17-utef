...
namespace X17
{
  ...
  class CircleFit3D
  {
    using EvalFn = void (*)(Int_t&, Double_t*, Double_t&, Double_t*, Int_t);

  private:
    static inline CircleFit3D* s_lastfit = nullptr; // Pointer to the last instance.

    std::vector<RecoPoint> m_fit_data; // An std::vector of reconstructed points to be fitted.
    Watched<double> m_total_charge;    // Total charge (or number of points in the track (can be weighted)).
    ...
    double m_alpha_min = -std::numbers::pi / 2;    // Minimal alpha bound for the fit [rad].
    double m_alpha_max = 3 * std::numbers::pi / 2; // Maximal alpha bound for the fit [rad].
  ...
  public:
    ...
    [[nodiscard]] std::vector<RecoPoint> GetData() const { return m_fit_data; }
    ...
    [[nodiscard]] TGraph2D* GetGraph(double step = 0.1, double dist = 0) const;
    ...
    /// @param middle Boolean flag indicating whether to use the middle field of the circle (true) or the average field
    /// (false).
    /// @return Kinetic energy of the particle.
    [[nodiscard]] double GetEnergy(const Field<Vector>& magfield, bool middle = true) const;
    ...
    [[nodiscard]] TGraph* GetEnergyGraph(const Field<Vector>& magfield, double step = 0.1) const;

    /// @brief Returns the sum of squared residuals of the fit.
    /// @return Sum of squared residuals.
    [[nodiscard]] double GetSumSq() const { return SumSq_(); }
  ...
  private:
    ...
    [[nodiscard]] Vector GetLinePoint_(double param, bool first_line) const;
    ...
    [[nodiscard]] Vector GetCirclePoint_(double varphi) const;

    /// @brief Updates the curve parameters based on the current m_orientation, m_length, m_radius, m_alpha, and
    /// m_phi_max values. This function calculates the new values of m_cos_theta, m_sin_theta, m_cos_phi, m_sin_phi,
    /// m_cos_alpha, m_sin_alpha, m_originc, m_normal, m_center, m_origin2, and m_orientation2. These values are used to
    /// update the properties of the curve.
    /// @note This function assumes that the m_orientation, m_length, m_radius, m_alpha, and m_phi_max values have
    /// already been set.
    void UpdateCurve_();
    ...
    [[nodiscard]] double LineSqDist_(RecoPoint point, bool first_line) const;
    ...
    [[nodiscard]] double CircleSqDist_(RecoPoint point) const;
    ...
    [[nodiscard]] double SqDistance_(RecoPoint point) const;

    /// @brief Calculates the sum of squared distances from each point in the m_fit_data vector to the curve. Each
    /// distance is scaled by the count/charge.
    /// @return Sum of squared distances from each point in m_fit_data to the curve.
    [[nodiscard]] double SumSq_() const;

    /// @brief Calculate the sum of squared distances for a given set of parameters.
    /// @param npar Number of parameters.
    /// @param gin Array of size npar used to store the first derivatives of the function with respect to the
    /// parameters.
    ...
    void EvalSumSq_(int& npar, double* gin, double& sumsq, double* par, int iflag);

    /// @brief A static wrapper function used to get the correct function pointer type needed for ROOT. Simply calls
    /// lastfit->_EvalSumSq().
    /// @param npar Number of parameters.
    /// @param gin Array of size npar used to store the first derivatives of the function with respect to the
    /// parameters.
    ...
    /// @param par Array of size npar that contains the values of the parameters.
    /// @param iflag Integer that can be used to control the behavior of the function.
    static void Eval_(int& npar, double* gin, double& sumsq, double* par, int iflag);
    ...
    [[nodiscard]] Vector GetAvgField_(const Field<Vector>& magfield, double step = 0.1) const;

    /// @brief Get the magnetic field at the point on the circle that passes through the middle of the X17 detector in
    /// the x direction.
    /// @param magfield The magnetic field vector field to sample from.
    /// @param tolerance The maximum deviation from the middle of the detector in the x direction for which to stop the
    /// binary search. Default is 0.0001.
    /// @return Magnetic field vector at the point on the circle that passes through the middle of the X17 detector in
    /// the x direction.
    [[nodiscard]] Vector GetMiddleField_(const Field<Vector>& magfield, double tolerance = 0.0001) const;
  };
} // namespace X17