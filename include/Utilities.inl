...
// X17 dependencies
#include "Utilities.h" // NOLINT(misc-header-include-cycle)
...
void ApplyThesisStyle(T* obj)
{
  ...
  else
  {
    ...
    {
      std::cout << "Warning: Can't apply thesis style for object of type " << typeid(obj).name() << '\n';
      return;
    }
    ...
    if (axes[2])
    {
      ...
      zmax            = std::max<double>(std::abs(zmin), zmax);
      zmax            = std::max<double>(zmax, 9);
      double z_offset = std::ceil(std::log10(zmax)) * 0.2 + 0.5;
      if (zmin < 0)
        z_offset += 0.2;
      axes[2]->SetTitleOffset(static_cast<float>(z_offset)); // previously 1.3 for times in 1000s of ns
    }
