...
namespace X17
{
  ...
  struct TrackMicro
  {
    ...
    TrackMicro(bool e, const std::vector<MicroPoint>& pts, Vector orig, Vector orient, double Ek,
               const std::vector<std::vector<DriftLinePoint>>& dlines)
    ...
    [[nodiscard]] double theta() const { return (180 / TMath::Pi()) * asin(orientation.z); }

    /// @brief Returns the azimuth angle phi of the track.
    /// @return Angle phi of the track [deg].
    [[nodiscard]] double varphi() const
    {
      return (180 / TMath::Pi()) * acos(orientation.x / cos(asin(orientation.z))) * sign(orientation.y);
    }
  ...
  struct TrackInfo
  {
    ...
    double cfit_nopads_energy_avg; // Circle fit energy (not account. for pads) [MeV] calc. using avg field (corrected).
    double cfit_pads_energy_mid;   // Circle fit energy (accounting for pads) [MeV] calculated using middle field.
    double cfit_pads_energy_avg;   // Circle fit energy (accounting for pads) [MeV] calc. using avg field (corrected).
    ...
    [[nodiscard]] double GetRKRelError() const { return 100 * (rkfit_energy - kin_energy) / kin_energy; }
