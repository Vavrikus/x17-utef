...
/// @return 0 if the fit is successful, -1 if the number of parameters is invalid, -2 if the number of points is too
/// small, -3 if the matrix is singular.
inline int LinearFit3D(const std::vector<FPoint>& data, const int p = 4, bool use_reco = false)
{
  const int n = static_cast<int>(data.size()); // number of points
  if (p < 2 || p > 4)
    return -1;
  if (n < p)
    return -2;
  ...
  for (int i = 0; i < p; ++i)
    std::cout << "   a" << i << " (" << param_vars[i] << ") = " << coeffs[i] << '\n';
