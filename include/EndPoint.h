...
namespace X17
{
  ...
  struct EndPoint
  {
    ...
    EndPoint(double x, double y, double z, double t)
      : point(x, y, z), t(t)
    {
    }
    ...
    /// @param t The final time [ns].
    EndPoint(Vector point, double t)
      : point(point), t(t)
    {
    }
    ...
    [[nodiscard]] EndPoint Square() const
    {
      return EndPoint(point.x * point.x, point.y * point.y, point.z * point.z, t * t);
    }
    ...
    [[nodiscard]] EndPoint SquareRoot() const
    {
      return EndPoint(std::sqrt(point.x), std::sqrt(point.y), std::sqrt(point.z), std::sqrt(t));
    }

    [[nodiscard]] Matrix<4, 1> ToVector4() const { return Matrix<4, 1>({ point.x, point.y, point.z, t }); }

    /// @brief Assignment operator.
    /// @param other The endpoint to be assigned.
    ...
    EndPoint operator/(double d) const { return EndPoint(point / d, t / d); }
    ...
    EndPoint operator+(EndPoint p2) const { return EndPoint{ point + p2.point, t + p2.t }; }
