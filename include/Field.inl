...
// X17 dependencies
#include "Field.h" // NOLINT(misc-header-include-cycle)
#include "Utilities.h"
...
namespace X17
{
  ...
  template <typename T>
  void Field<T>::GetPointIndices_(double x, double y, double z, int& xi, int& yi, int& zi) const
  {
    ...
    if (x < m_xmin || x > m_xmax || y < m_ymin || y > m_ymax || z < m_zmin || z > m_zmax)
    {
      ...
      std::string bounds = " Bounds: x: [" + std::to_string(m_xmin) + ", " + std::to_string(m_xmax) + "], y: ["
                           + std::to_string(m_ymin) + ", " + std::to_string(m_ymax) + "], z: [" + std::to_string(m_zmin)
                           + ", " + std::to_string(m_zmax) + "].";
    ...
    if (std::isnan(x) || std::isnan(y) || std::isnan(z))
      throw std::runtime_error("Cannot read field at NaN coordinates.");
      ...
      m_def_value(def_value)
  {
    ...
    this->m_num_x_cells = iround((max_corner.x - min_corner.x) / step) + 1;
    this->m_num_y_cells = iround((max_corner.y - min_corner.y) / step) + 1;
    this->m_num_z_cells = iround((max_corner.z - min_corner.z) / step) + 1;
  ...
  T Field<T>::GetField(double x, double y, double z) const
  {
    ...
    const double dx = std::abs((x - x_grid) / m_step_size);
    const double dy = std::abs((y - y_grid) / m_step_size);
    const double dz = std::abs((z - z_grid) / m_step_size);
