...
namespace X17
{
  ...
  public:
    ...
    /// @brief Constructs a Field object with the given dimensions and step size, and sets the default value for all
    /// points.
    ...
    T& at(int xi, int yi, int zi) { return m_data[zi * m_num_y_cells * m_num_x_cells + yi * m_num_x_cells + xi]; }
    ...
    /// @param zi The z index of the data.
    /// @return Value of the data at the given index.
    [[nodiscard]] const T& at(int xi, int yi, int zi) const
    {
    ...
    /// @return Value of the data at the given index.
    [[nodiscard]] const T& at(const int (&i_arr)[3]) const { return this->at(i_arr[0], i_arr[1], i_arr[2]); }

    /// @brief Get the minimum x coordinate of the field.
    /// @return Minimum x coordinate.
    [[nodiscard]] double GetXMin() const { return m_xmin; }

    /// @brief Get the maximum x coordinate of the field.
    /// @return Maximum x coordinate.
    [[nodiscard]] double GetXMax() const { return m_xmax; }

    /// @brief Get the minimum y coordinate of the field.
    /// @return Minimum y coordinate.
    [[nodiscard]] double GetYMin() const { return m_ymin; }

    /// @brief Get the maximum y coordinate of the field.
    /// @return Maximum y coordinate.
    [[nodiscard]] double GetYMax() const { return m_ymax; }

    /// @brief Get the minimum z coordinate of the field.
    /// @return Minimum z coordinate.
    [[nodiscard]] double GetZMin() const { return m_zmin; }

    /// @brief Get the maximum z coordinate of the field.
    /// @return Maximum z coordinate.
    [[nodiscard]] double GetZMax() const { return m_zmax; }

    /// @brief Get the step size of the field.
    /// @return Step size.
    [[nodiscard]] double GetStep() const { return m_step_size; }

    /// @brief Get the number of x cells.
    /// @return Number of x cells.
    [[nodiscard]] int GetXCells() const { return m_num_x_cells; }

    /// @brief Get the number of y cells.
    /// @return Number of y cells.
    [[nodiscard]] int GetYCells() const { return m_num_y_cells; }

    /// @brief Get the number of z cells.
    /// @return Number of z cells.
    [[nodiscard]] int GetZCells() const { return m_num_z_cells; }

    /// @brief Returns the number of grid cells.
    /// @return Total number of grid cells.
    [[nodiscard]] int GetNCells() const { return this->m_num_x_cells * this->m_num_y_cells * this->m_num_z_cells; }
    ...
    {
      int xi, yi, zi;
      GetPointIndices_(x, y, z, xi, yi, zi);
    ...
    /// @return Pointer to the field value at the specified coordinates.
    /// @throws std::out_of_range if the specified coordinates are out of bounds.
    T* GetPoint(const Vector& position) { return this->GetPoint(position.x, position.y, position.z); }
    ...
    /// @throws std::out_of_range if the specified coordinates are out of bounds.
    void SetPoint(double& x, double& y, double& z, const T& new_value) { *this->GetPoint(x, y, z) = new_value; }
    ...
    [[nodiscard]] T GetField(double x, double y, double z) const;
    ...
    [[nodiscard]] T GetField(Vector vec) const { return GetField(vec.x, vec.y, vec.z); }
  ...
  private:
    ...
    /// @throws std::out_of_range if the specified coordinates are out of bounds.
    void GetPointIndices_(double x, double y, double z, int& xi, int& yi, int& zi) const;
  ...
  Field<Vector>* LoadField(const char* filename, Vector min_corner, Vector max_corner, double step,
