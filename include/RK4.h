...
namespace X17
{
  ...
  class RK4
  {
    using VectorN = Matrix<N, 1>;
    using VecFn   = std::function<void(double, const VectorN&, VectorN&)>;
    using EndFn   = std::function<bool(double, const VectorN&)>;

  private:
    VecFn m_dif_eq;    // The function object that returns the derivative of the dependent variable with respect to the
    ...
    std::vector<VectorN>
      m_results; // A vector that stores the values of the dependent variable at different points in the integration.
  ...
  public:
    ...
    /// @param equation The function object that returns the derivative of the dependent variable with respect to the
    /// independent variable.
    ...
    [[nodiscard]] const std::vector<VectorN>& GetResults() const { return m_results; }

    /// @brief Returns the number of points in the integration.
    /// @return The number of points in the integration.
    [[nodiscard]] int GetSize() const { return m_results.size(); }
  ...
  void EMMotion(const Field<Vector>& magfield, bool electron, double tau, const Matrix<8, 1>& params,
                Matrix<8, 1>& output);
  ...
  /// @brief Checks if the particle is out of the first sector TPC and outside a rectangular volume in front of it (x <
  /// xmin).
  /// @param tau Proper time elapsed (independent variable) [s].
  /// @param params Vector of position and four-velocity of the particle.
  ...
  RK4<8>* GetTrackRK(const Field<Vector>& magfield, bool electron, double step, double kin_en, Vector origin,
                     Vector orientation, bool big_volume = false);
  ...
  /// @return The RKPoint object representing the position (with time) of the particle at the specified index on the
  /// trajectory.
  RKPoint GetTrackRKPoint(const RK4<8>* track, int index);

  /// @brief Computes the square of the distance between a point and a point on the computed trajectory at a specified
  /// index.
  /// @param track Pointer to a simulated RK4 trajectory.
  ...
  /// @return The square of the distance between the specified point and the point on the trajectory at the specified
  /// index.
  double GetTrackRKSqDist(const RK4<8>* track, int index, Vector point);

  /// @brief Computes the scaled derivative of the square of the distance between a point and the point on the computed
  /// trajectory with respect to the index.
  /// @param track Pointer to a simulated RK4 trajectory.
  ...
  /// @return The derivative of the square of the distance between the specified point and the computed trajectory at
  /// the specified index with respect to the index.
  ...
  /// @brief Calculates the squared distance between the given point and the curve and returns the closest point on the
  /// curve.
  /// @param track Pointer to a simulated RK4 trajectory.
  /// @param point The point to calculate the distance to.
  ...
  /// @brief Returns a TGraph2D object representing the track of a particle obtained from a Runge-Kutta 4th order
  /// simulation.
  ...
  {
  private:
    static inline RKFit* s_lastfit = nullptr; // A static pointer to the last instance of RKFit class created.
  ...
  public:
    ...
    RKFit(Field<Vector>* magfield, bool electron, double step, Vector origin, Vector orientation,
          const std::vector<RecoPoint>& fit_data, bool big_volume = false);

    /// @brief Getter for the kinetic energy.
    /// @return Kinetic energy [eV].
    [[nodiscard]] double GetEnergy() const { return m_kin_en; }

    /// @brief Getter for the kinetic energy error.
    /// @return Error of the kinetic energy [eV].
    [[nodiscard]] double GetEnergyError() const { return m_e_err; }
    ...
    /// @brief Fits the Runge-Kutta object to the given data using the MINUIT fitter with specified maximum number of
    /// iterations and toleration.
  ...
  private:
    ...
    void SumSqDist_(int& npar, double* gin, double& sumsq, double* par, int iflag);
    ...
    {
      s_lastfit->SumSqDist_(npar, gin, sumsq, par, iflag);
    }
  };
} // namespace X17
