...
namespace X17::Color
{
  ...
  {
    float red_NDC   = 1.0F + (static_cast<float>(R) / 255.0F - 1) * saturation / 100.0F;
    float green_NDC = 1.0F + (static_cast<float>(G) / 255.0F - 1) * saturation / 100.0F;
    float blue_NDC  = 1.0F + (static_cast<float>(B) / 255.0F - 1) * saturation / 100.0F;
    ...
    if (find_closest)
    {
      ...
      {
        TColor* color = static_cast<TColor*>(gROOT->GetListOfColors()->At(i));
        if (!color)
          continue;
