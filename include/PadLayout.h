...
namespace X17
{
  ...
  public:
    ...
    [[nodiscard]] virtual double GetPadHeight(int i, bool effective = false) const = 0;
    ...
    virtual void GetPadCorners(int i, double& out_xlow, double& out_ylow, double& out_xhigh, double& out_yhigh,
                               bool nogaps = false) const = 0;

    /// @brief Function for retrieving the number of a pad (channel) effective area hit for given position (currently
    /// slow approach, may need optimization in future).
    ...
    [[nodiscard]] virtual int GetPad(double x, double y) const = 0;
  ...
  inline int triangle_row(int columns, int index)
  {
    return ceil((2 * columns + 1 - std::sqrt(pow((2 * columns + 1), 2) - 8 * index)) / 2);
  }
  ...
  public:
    ...
    /// @param effective If true, gap width will be added to the height.
    /// @return The height of the i-th pad.
    [[nodiscard]] double GetPadHeight(int i, bool effective = false) const override;
    ...
    /// @param out_yhigh Will be set to the y-coordinate of the top right corner.
    /// @param nogaps If true, the gaps are evenly divided between neighbouring pads.
    void GetPadCorners(int i, double& out_xlow, double& out_ylow, double& out_xhigh, double& out_yhigh,
                       bool nogaps = false) const override;
    ...
    /// @param y The y-coordinate [cm].
    /// @return Number of pad containing the given point (-1 if no pad contains this point).
    [[nodiscard]] int GetPad(double x, double y) const override;
