...
namespace X17
{
  ...
  struct Matrix
  {
    ...
    /// If the specified row or column is out of bounds, returns a reference to the first element and prints an error
    /// message.
    /// @note If DEBUG is defined, this method checks if the specified row and column are within bounds and prints an
    /// error message if not. If DEBUG is not defined, no bounds checking is performed for efficiency.
    [[nodiscard]] const double& at(int row, int column) const;
    ...
    /// @return An array representing the specified column of the matrix, or an empty vector if the index is out of
    /// bounds.
    [[nodiscard]] std::array<double, M> GetColumn(int c) const;
