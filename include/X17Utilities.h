...
namespace X17
{
  namespace constants
  {
    ...
    constexpr double yintersect
      = ylow - yxslope * xmin; // TPC slanted side (y positive) y-axis intersection (extrapolated) [cm]
    ...
    constexpr double pad1_offset
      = -0.10; // Vertical offset [cm] of corner of first pad from top right corner of trapezoid
    ...
    const double X_theta_max = (180 / M_PI) * std::atan((win_height / 2) / xmin); // The maximal simulated theta [deg].
    const double X_theta_min = -X_theta_max;                                      // The minimal simulated theta [deg].
    const double X_phi_max   = (180 / M_PI) * std::atan((win_width / 2) / xmin);  // The maximal simulated phi [deg].
    const double X_phi_min   = -X_phi_max;                                        // The minimal simulated phi [deg].
    constexpr double X_E_max = 13;                                                // The maximal simulated energy [MeV].
    constexpr double X_E_min = 3;                                                 // THe minimal simulated energy [MeV].
  } // namespace constants

  /// @brief Returns true if given point is in the first sector TPC (containing positive x-axis) of the detector
  /// (trapezoidal prism).
  /// @param x The x-coordinate [cm].
  /// @param y The y-coordinate [cm].
  /// @param z The z-coordinate [cm].
  /// @param dist Specifies the minimal distance from within the TPC walls except the bottom anode wall (makes volume
  /// smaller).
  ...
  /// @brief Returns minimal (magnetic) field angle to electric field (along positive y-axis) inside first sector TPC
  /// volume.
  /// @param magfield Magnetic field data.
  ...
  void DrawTrapezoid(short width = 1, bool yxformat = true);
