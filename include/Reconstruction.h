...
namespace X17
{
  ...
  /// @return A 2D array containing the interpolation coefficients for each of the x, y, and t coordinates of the
  /// MapPoint values.
  ...
  /// @brief Does the pseudocell in the inverse grid given by the indices of the map contain the point in its
  /// circumscribed box?
  ...
  /// @return A RecoPoint object that represents the reconstructed point in 3D space. (x0,y0,z0)
  inline RecoPoint Reconstruct(const Field<MapPoint>& map, MicroPoint point)
  {
    return Reconstruct(map, point.end);
  }

  /// @brief Estimates the distance between two points, with given coordinates and time values in a SensorData object.
  /// The time value is weighted by a factor.
  ...
  /// @brief Reconstructs a point with given final coordinates using the map of ion electron drift. Uses gradient
  /// descend. Old function.
