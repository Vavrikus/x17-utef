...
// X17 dependencies
#include "NSpline.h" // NOLINT(misc-header-include-cycle)
...
namespace X17
{
  ...
  template <int N>
  double NSpline<N>::Eval(double* x, double* par) const // NOLINT(readability-non-const-parameter)
  {
    // Fit parameters:
    ...
    if (m_fix_der_start)
      par[2ULL * N] = m_der_start;
    if (m_fix_der_end)
      par[2ULL * N + 1] = m_der_end;

    Double_t b1 = par[2ULL * N];
    Double_t e1 = par[2ULL * N + 1];
  ...
  template <int N>
  NSpline<N>::EvalFn NSpline<N>::GetEval() const
  {
    return [this](double* a, double* b) { return this->Eval(a, b); };
  }
  ...
  TSpline3* FitSplines(TGraph* graph, double min, double max)
  {
    ...
    for (int i = 0; i < nodes; i++)
      nodes_x[i] = min + (i / (nodes - 1.0)) * (max - min);
    ...
    for (int i = nodes; i < ibeg1; i++)
      nodes_y[i - nodes] = fit_pars[i];
    double beg1 = fit_pars[ibeg1];
    double end1 = fit_pars[ibeg2];
