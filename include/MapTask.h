...
{
private:
  const X17::DriftMap& m_map; // A pointer to the ionization electron map.
...
public:
  ...
  virtual void XYZ_Loop(double x, double y, double z,
                        X17::MapPoint current) // NOLINT(performance-unnecessary-value-param)
  {
  }
  ...
  /// @param x The x coordinate of the current map point.
  /// @param current The current map point.
  virtual void ZX_Loop(double z, double x, const X17::MapPoint& current) {}
...
protected:
  ...
  [[nodiscard]] const X17::DriftMap& GetMap_() const { return m_map; }
};