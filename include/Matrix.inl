...
// X17 dependencies
#include "Matrix.h" // NOLINT(misc-header-include-cycle)
...
namespace X17
{
  ...
  void Matrix<M, N>::SwapRows(int row1, int row2)
  {
    ...
    if (row1 > -1 && row1 < M && row2 > -1 && row2 < M)
    {
      for (int c = 0; c < N; c++)
      {
        std::swap(this->at(row1, c), this->at(row2, c));
      }
    }

    else
      std::cerr << "ERROR: Invalid matrix row number.\n";
  }
  ...
  void Matrix<M, N>::Reduce()
  {
    for (int c = 0; c < M; c++)
    {
      ...
      const double first = this->at(c, c);
      if (first == 0)
