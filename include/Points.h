...
namespace X17
{
    ...
    struct EndPoint
    {
        ...
        EndPoint SquareRoot() { return EndPoint(std::sqrt(point.x),std::sqrt(point.y),std::sqrt(point.z),std::sqrt(t)); }

        Matrix<4,1> ToVector4() { return Matrix<4,1>({point.x,point.y,point.z,t}); }
    ...
    /// @brief Calculates the covariance matrix of a set of points.
    /// @param values The set of points.
    /// @param average The average of the points.
    /// @return The covariance matrix of the points.
    Matrix<4,4> CovarianceMatrix(std::vector<EndPoint> values, EndPoint average);

    /// @brief Calculates the squared Mahalanobis distancea of a set of points.
    /// @param values The set of points.
    /// @param average The average of the points.
    /// @param cov The covariance matrix of the points.
    ...
    struct MapPoint
    {
        ...
        MapPoint(EndPoint p, Matrix<4,4> m) : point(p), cov_mat(m) { }

        /// @brief Diagonalize the covariance matrix and store the eigenvalues and eigenvectors in eigen_values and eigen_vectors.
        /// @param eigen_values To be filled with eigenvalues of the covariance matrix.
        /// @param eigen_vectors To be filled with eigenvectors of the covariance matrix.
        /// @param no_z If true, the z-coordinate is not included in the diagonalization.
        void Diagonal(TVectorD& eigen_values, TMatrixD& eigen_vectors, bool no_z = true) const;
        ...
        double xdev(int N = 100) const { return StdevBiasFactor(N)*std::sqrt(cov_mat.at(0,0)); }

        /// @brief Getter for the y variable deviation.
        /// @return The y-coordinate deviation [cm].
        double ydev(int N = 100) const { return StdevBiasFactor(N)*std::sqrt(cov_mat.at(1,1)); }

        /// @brief Getter for the z variable deviation.
        /// @return The z-coordinate deviation [cm].
        double zdev(int N = 100) const { return StdevBiasFactor(N)*std::sqrt(cov_mat.at(2,2)); }

        /// @brief Getter for the t variable deviation.
        /// @return Time deviation [ns].
        double tdev(int N = 100) const { return StdevBiasFactor(N)*std::sqrt(cov_mat.at(3,3)); }
        ...
        {
            return MapPoint{point + p2.point, cov_mat + p2.cov_mat};
        }
    ...
    {
        return MapPoint{d*p.point, d*p.cov_mat};
    }
