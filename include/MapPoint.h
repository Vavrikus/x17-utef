...
#include "Rtypes.h"
#include "TEllipse.h"
#include "TMatrixD.h" // IWYU pragma: keep
#include "TRandom3.h"
#include "TVectorD.h" // IWYU pragma: keep
...
namespace X17
{
  /// @brief A struct for storing the results of map simulation (multiple ionization electrons with same initial
  /// coordinates).
  struct MapPoint
  {
    ...
    MapPoint()
      : n(0), point(), cov_mat(0), eigen_vecs(4, 4), eigen_vals(4)
    {
    }
    ...
    MapPoint(int n, EndPoint p, Matrix<4, 4> m)
      : n(n), point(p), cov_mat(m), eigen_vecs(4, 4), eigen_vals(4)
    {
    }

    /// @brief Diagonalize the covariance matrix and store the eigenvalues and eigenvectors in eigen_values and
    /// eigen_vectors.
    ...
    [[nodiscard]] TEllipse* GetErrorEllipse(int skip_index, double sigma = 1.0, bool x_bigger = false) const;
    ...
    /// @return Time [ns].
    [[nodiscard]] double t() const { return point.t; }

    /// @brief Getter for the x variable deviation.
    /// @return The x-coordinate deviation [cm].
    [[nodiscard]] double xdev() const { return StdevBiasFactor(n) * std::sqrt(cov_mat.at(0, 0)); }

    /// @brief Getter for the y variable deviation.
    /// @return The y-coordinate deviation [cm].
    [[nodiscard]] double ydev() const { return StdevBiasFactor(n) * std::sqrt(cov_mat.at(1, 1)); }

    /// @brief Getter for the z variable deviation.
    /// @return The z-coordinate deviation [cm].
    [[nodiscard]] double zdev() const { return StdevBiasFactor(n) * std::sqrt(cov_mat.at(2, 2)); }

    /// @brief Getter for the t variable deviation.
    /// @return Time deviation [ns].
    [[nodiscard]] double tdev() const { return StdevBiasFactor(n) * std::sqrt(cov_mat.at(3, 3)); }
  ...
  inline MapPoint operator*(double d, const MapPoint& p)
  {
