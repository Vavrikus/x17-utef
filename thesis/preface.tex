...
\renewcommand{\thefigure}{\thechapter.\arabic{figure}}
	A~\ac{TPC} \red{[refs]} is a~type of gaseous detector that detects charged particle trajectories by measuring the~positions and drift time of ions created in the~gas. The~energies of these particles can be inferred from the~curvatures of their trajectories in the~magnetic field (\orange{specific field inside the~\ac{TPC}}).
	...
	\label{sec:ATOMKI}
		Many different theories propose the~existence of \emph{new light boson(s)} that are weakly coupled to ordinary matter~\cite{dark}. These particles are potential dark matter candidates and could contribute to a~solution of other issues with the~Standard Model, such as the~strong CP~problem and the~anomalous muon magnetic moment. \orange{Mass range of axions?}
		...
		\subsection{ATOMKI Measurements}
			Historically, there were several measurements of the~\ac{IPC} in nuclear transitions in~\iso{Be}{8} at Institute für Kernphysik (Frankfurt)~\cite{ikf1996,ikf1997,ikf2001} and at ATOMKI (Debrecen, Hungary)~\cite{atomki2008,atomki2012} resulting in different anomalies with invariant mass in the range $5-15$~MeV. This prompted a~development of a~better spectrometer at ATOMKI.
