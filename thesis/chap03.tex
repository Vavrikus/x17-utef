...
\label{sec:track}
	...
	\label{sec:map}
		...
		\subsection{Map Simulation}
			...
			\red{The obtained map is then stored in a~custom class template \textit{Field}, could expand on that. Maybe earlier, since the same template is used for the magnetic field. Extra space on the sensor. Using qsub (not sure if important).\\}
