...
\label{sec:track}
	...
	\label{sec:trackfirst}
		As the~first step, we tried to reconstruct an~electron track considering a~special set of initial parameters. The~origin (\orange{starting point?}) of the~particle is given by the~origin of our coordinate system and its initial direction is given by the~positive $x$\nobreakdash-axis. This means the~magnetic field of our detector is perpendicular to the~momentum of the~particle at all times, and we can reduce the~problem to two-dimensional space. As an~example, we use a~track simulated using the~microscopic simulation with the~kinetic energy 8~MeV. The~gas composition used in this simulation is 90:10 Ar:CO$_2$. \red{Might be better to describe this track in Section~\ref{sec:microsim} and reference it here.}
