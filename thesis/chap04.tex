...
\label{sec:energy}
	...
	\label{sec:clines}
		...
		\subsection{Two-dimensional fit}
			In the~2D case, the~fitted function used for the~electron track\footnote{Electron tracks bend towards negative~$z$, we need to use the~upper part of the~circle} described in Section~\ref{sec:cspline} (\red{or in microsim? one specific track at the time, technically this function doesn't work for a curvature that gets outside of the semicircle}) is defined as follows: \red{Maybe describe this track that we used at the~beginning somewhere earlier (section microscopic simulations \textrightarrow~Testing track?) so that it is easier to refer to it in multiple sections. It is not part of the~early GitHub commits, so maybe it won't be possible to create exact replicas of the~images, but they should be at least very similar.}
				\begin{equation}
					\label{eq:clines2d}
					z(x) = \begin{cases}
								a_1x+b_1 & x<x_1\\
		...
		\subsection{Three-dimensional fit}
			\red{Explain the geometry and least square method used for the~3D fit. Tested on a~Runge-Kutta sample, and with microscopic tracks + map simulation.}
			...
			We assume that the~initial position $\mathbf{X}_0 = (x_0,y_0,z_0)$ and direction $\theta,\varphi$ (\red{spherical angles as in Section~\ref{sec:coor}}) are known, since this information will be provided by \ac{TPX3} and \ac{MWPC} layers. \red{We could further refine it at the~end of the current algorithm with some kind of global fit (all detector layers).} The fit then has four free parameters (\red{figure}):
				\begin{itemize}[nosep]
	...
	\section{Runge-Kutta Fit}
		...
		with minimal and maximal $\alpha$ differing by less than~$\pi$ (i.e., the~track changes direction by less than $180^\circ$), then all intersections of the~track's normals must lie on the~side of the~evolute closer to the track (\red{not obvious?, sometimes the sides are opposite?}). At the~same time, the~intersection must lie in the half planes given by the~normals at the~beginning and the~end of the~curve and pointing away from the~curve. Together, these three boundaries define a closed shape that will lie outside of the~\ac{OFTPC} for a~typical track in our detector.
			\begin{figure}
				...
				\end{subfigure}
				\caption{\red{some provisional figures}}
				\label{fig:rkdemo2}
			\end{figure}
		...
		\subsection{Testing on a~microscopic sample}
			...
			\red{Initial parameters of the~HEED track (also should be in the first testing track $\rightarrow$ subsection of microsim?). Initial parameters set in the~circle fit (if electron set alpha one way, otherwise other way) and parameter bounds.}