	...
	\label{sec:clines}
		...
		\subsection{Three-dimensional fit}
			...
			If the parameter value is outside of its bounds defined above, we take the boundary value instead. The distance to the second line is computed likewise. For the circular arc\orange{~(specific circular arc in the fit)}, we find the closest point\orange{~(on the arc)} by projecting the center connecting line onto the arc plane:
				...
				\end{gather}
			If the point $\mathbf{X}_{PC}$ lies outside of the arc, distance to the closest endpoint is taken instead.\red{~Sometimes (rarely?) the distance might be underestimated when the point is closer to the circle, the arc is a part of, than to the curve. I tried to fix this, but it breaks the fit and makes the results worse.} The shortest distance out of $d_{P1},d_{PC},d_{P2}$ is then taken as the distance to the curve.\red{~TVirtualFitter+MIGRAD, maximal num of iterations, toleration. Different uncertainties in $x,y,z$ not taken into account.}
			...
			\subsection{Testing on a~Runge-Kutta sample}
				...
				\begin{figure}
					...
					\begin{subfigure}[t]{\textwidth}
						...
						\caption{Overall relative error for both magnetic field selection methods. Freedman-Diaconis rule was used for bin width selection~\cite{FreedmanDiac}.}
					\end{subfigure}
